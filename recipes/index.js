...
// Returns the recipe template given the recipe
...
// Returns the tags template given the tags
...
// Returns the rating template given the rating
...
// Returns the template for the stars given the rating
function starsTemplate(rating) {
    ...
    for (let i = 1; i <= 5; i++) {
        ...
        template += `<span aria-hidden="true" class="icon-star">${star}</span>`;
    }
    return template;
}

// Renders the given recipes
function renderTemplates(recipes) {
    recipesElement.innerHTML = "";
    recipes.forEach(recipe => {
        recipesElement.innerHTML += recipeTemplate(recipe);
    });
}

// Render recipes that fit the search text
searchForm.addEventListener("submit", event => {
    event.preventDefault();

    const searchText = searchForm.searchText.value.toLowerCase();

    // Filter for recipes with the name in search text
    ...
    renderTemplates(filteredRecipes);
});
...
function init() {
    ...
    renderTemplates([recipesData[randomIndex]]);
}
